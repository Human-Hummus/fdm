...
proc parser*(input: seq[token]): seq[node] =
  ...
  while x < input.len:
    ...
    if input[x].kind == tokentype.text:
      ...
      output.add node(kind: nodetype.array_access, subvalues: @[preval],
          fncontents: parser(post_tokens), pos: pos)
      ...
      if x+1 < input.len and input[x+1].kind == tokentype.equals:
        ...
        var
          buffer: seq[token] = @[]
          depth = 0
        while x < input.len and not (depth == 0 and input[x].kind ==
            tokentype.semi):
      ...
      if input[x].value == "func":
        ...
        var name = input[x].value
        x+=1
        if x >= input.len or input[x].kind != tokentype.paren or input[
            x].value != "(":
        ...
        if x+1 < input.len and input[x+1].kind in @[paren, curly] and input[
          ...
          if input[x].kind == tokentype.paren:
            x+=1
            while x < input.len and not (input[x].kind == tokentype.paren and
                input[x].value == ")" and depth == 0):
          ...
          arg_buffer = @[]
          depth = 1
          if x < input.len and input[x].kind == tokentype.curly and input[
              x].value == "{":
            x+=1
            while x < input.len and depth > 0:
          ...
          output.add node(kind: nodetype.function_call, name: name,
              fnvars: args, fncontents: parser(fncontents), pos: pos)
          x-=1
        else:
